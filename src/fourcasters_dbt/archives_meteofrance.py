...
from fourcasters_dbt.incendie import (
    ...
    preparer_publications,
)

logger = logging.getLogger(__name__)
TABLE_LANDING_ARCHIVES = f"{PROJET_GCP}.meteofrance_landing.meteo_forets_archives"
...
def controler_annees(annees: list[int]) -> None:
    ...
    if not annees or annees_invalides:
        raise ValueError(
...
def telecharger_archive(annee: int) -> pd.DataFrame:
    ...
    logger.info("⬇️ Téléchargement de l'année %s...", annee)

    reponse = recuperer_reponse(url, timeout=TIMEOUT_TELECHARGEMENT)
    ...
    logger.info("Année %s : %s lignes récupérées.", annee, len(donnees))
    return donnees
...
def preparer_archives(donnees: pd.DataFrame) -> pd.DataFrame:
    ...
    return preparer_publications(archives.drop(columns="annee_archive"))
...
def importer_archives() -> None:
    ...
    logger.info("🔥 Import de l'historique Météo des forêts")
    ...
    logger.info("Années : %s", ", ".join(str(annee) for annee in annees))
    logger.info("Publications : %s", donnees["reference_time"].nunique())
    logger.info("Lignes : %s", len(donnees))
    ...
    logger.info("☁️ Fichier envoyé : %s", adresse_gcs)

    client_bigquery = bigquery.Client(project=PROJET_GCP)
    preparer_datasets_bigquery(client_bigquery)
    charger_parquet_bigquery(adresse_gcs, TABLE_LANDING_ARCHIVES, len(donnees))
    fusionner_historique_bigquery(client_bigquery, TABLE_LANDING_ARCHIVES)
    logger.info("✅ Import de l'historique terminé.")
